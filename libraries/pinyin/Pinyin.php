 ...
 *
 * This version includes an optimization from WEI Yongming (vincent@minigui.org).
 */
...
class Pinyin
{
	...
	/**
	 * OPTIMIZATION BY WEIYM
	 *
	 * Use APC cache to cache the dictionary. This speeds up the translation about 0.5 times.
	 * The elapsed time down from 0.25 seconds to 0.16 seconds.
	 */
	protected static $mCacheObj = null;
	protected static $mCacheGetFunc = null;
	protected static $mCacheSetFunc = null;
	protected static $mCacheKey;

	public static function set_real_cache ($cache_info)
	{
		self::$mCacheObj = $cache_info['cache_obj'];
		self::$mCacheGetFunc = $cache_info['cache_get_func'];
		self::$mCacheSetFunc = $cache_info['cache_set_func'];
		self::$mCacheKey = 'zh-pinyin-table';
	}
     ...
     * @return array
     */
    protected function loadFromCache($dictionary_file)
    {
		if (self::$mCacheGetFunc) {
			$cache_get_func = self::$mCacheGetFunc;
			$dictionary = $cache_get_func (self::$mCacheObj, self::$mCacheKey);
			if ($dictionary === FALSE) {
				$dictionary = include $dictionary_file;

				if (self::$mCacheSetFunc) {
					$cache_set_func = self::$mCacheSetFunc;
					$cache_set_func (self::$mCacheObj, self::$mCacheKey, $dictionary, 43200);
				}
			}
			...
			return include $dictionary_file;
		}
    }
	/**
	 * END OF OPTIMIZATION BY WEIYM
	 */
